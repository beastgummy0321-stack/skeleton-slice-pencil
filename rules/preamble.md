...
- A doc an agent writes is disposable unless it is `CONTEXT.md` or an ADR: it names on line one what makes it dead, and is born under a gitignored working path. Promoting one into the tracked tree is a user decision — the `doc-budget` gate holds the total, and only the user raises it.
