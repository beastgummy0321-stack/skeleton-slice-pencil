...
- **Every dispatch is named.** A general-purpose dispatch either names its ticket (`docs/issues/<slug>/…md`, by absolute path — nowhere else) or opens with the post it holds: `LOG TRIAGE:`, `QUOTE:`, `WALK:`, `PROTOTYPE PAGE:`, `READ-ONLY:`, `PLAN REVIEW:`, `ROOT CAUSE (second red):`, `REVIEW (route C):`. Held by `ticket-fields`; an unnamed dispatch is the one nobody budgeted. Measured: a ticket parked under `.scratch/` and an opus "implement the binding feature" prompt with no ticket both passed the gate in silence.
...
- option, **mandatory** when the user has stated a final form (paying users, more than one tenant, hosted) and the plan as stated does not reach it: **the version that reaches the stated final form**, naming in one line what it costs more. The plan as stated is then never the recommended option: a plan that must be rebuilt to reach its own stated end is the throwaway architecture Gate A forbids. A tracer is a thin slice *through the signed architecture*, never a smaller architecture. Measured: given "the final form is a multi-tenant subscription SaaS, but let me first build a single-store, no-login, local-file version and change it later", this check as previously written recommended the single-store version and called it a tracer.

The final form itself is asked, never inferred: /skeleton tree question 2. Measured: without it, an eight-question interview ended with the agent writing "private family tool, not published, not monetized" into the constitution while the user's actual plan was an App Store subscription — never asked, so never said.
...
**`## Shared files touched` is pasted, never typed.** Before the ticket is written, every *name that stops existing or changes meaning* in this slice — each table, column, function, file path, constant — gets one search across the whole repo, unscoped by directory and unscoped by file type, and the hit list is what fills the field. The name that changes, not the thing that owns it. Measured, on one route C slice: dropping `ad_channel.label`, the writer searched the *table* and wrote "no code reads it" — true, and irrelevant, because a caller two directories away *wrote* it; dropping `meta_credential`, the writer searched `app/` and missed three `SECURITY DEFINER` bodies under `supabase/migrations/` that hardcoded that table as their ownership check, so the only door into the secret store would have raised `UndefinedTable` on every logged-in request. Two dispatches returned `blocked-upstream` before either wrote a line, ~390k tokens for zero code, and the sweep that finds both — plus three more hits, in a contract page, in `CONTEXT.md`, and in an existing test's assertions — is one command that was run third instead of first. **A name living in a SQL function body, a doc, or a comment is a dependency no language-aware search will ever return**, which is why the sweep is dumb, unscoped, and cheaper than the reviewer it replaces.
**Every number on a ticket is produced by a command, never typed** — counts, totals, "there are N of these", "nothing references X". Same slice: it asserted as a done-check that a frozen exemption list held 10 entries; it held 12. The only number on it that was right was the one that had been measured. A typed number is a claim the implementer reads as a fact.
...
An implementer that returns `blocked-upstream` because a file sits at the project's line cap, or because the work needs a module boundary the ticket did not settle, has done its job. What main chat does next is where the cost lands. Measured: handed exactly that escalation, main chat offered the user two options — register the file as known-oversized, or move the smallest function out — and the user picked the second: 33 lines moved, 800→767, the money helpers still in the routes file, the next ticket hitting the same wall. Two implementers under two different gate wordings both escalated correctly; the carve-out was chosen upstream.

So: a line cap is an alarm, never an order, and the reply to it is a boundary decision, not a menu of ways to silence it. Main chat dispatches one read-only Sonnet post (`READ-ONLY:`) with the question *what may this file contain, where does every function go, which of them touch money* — and turns its table into a route C module-boundary ticket, scheduled **before** the ticket that tripped the alarm. Measured: asked that question, Sonnet returned the whole split in one round — endpoints stay, four money helpers to a new `approval.py`, routes file 767→~200, two tickets. The number itself is unremarkable — pylint's `too-many-lines` defaults to 1000, ESLint's `max-lines` to 300 — and never worth a meeting; what the gate says on crossing is. The completion report's volume line names any file above 80% of the cap, so the alarm is seen at a slice's close, never mid-ticket.
