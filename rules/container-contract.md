...
8. **The docs are one tree.** `CONSTITUTION.md` (≤20 lines) is the single entry; `BOARD.md` points to it; everything else lives under `docs/` — `CONTEXT.md`, `adr/`, one page per module in `contracts/<module>.md`, tickets in `issues/<slug>/` (gitignored, deleted on completion).
