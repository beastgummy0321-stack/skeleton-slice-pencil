...
7. **The docs are one tree.** `CONSTITUTION.md` (≤20 lines) is the single entry; `BOARD.md` points to it; everything else lives under `docs/` — `CONTEXT.md`, `adr/`, one page per module in `contracts/<module>.md`, tickets in `issues/<slug>/` (gitignored, deleted on completion).
   ...
   A dispatch prompt carries exactly: constitution summary + the one ticket + the contract pages the ticket names + the ADR index rows for the paths it touches (`node "{{PLUGIN_ROOT}}/hooks/adr-index.mjs" <path…>`). An agent never hunts for docs, and never reads an ADR body that governs nothing it is touching.
